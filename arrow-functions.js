...
const calculateAverage = (num1, num2, num3) => (num1 + num2 + num3) / 3;
...
const evaluateScores = (score1, score2, score3) => {
    ...
    if (max >= 90) {
        return `Excellent work! Top score: ${max}`;
    } else if (max >= 70) {
        return `Good job! Top score: ${max}`;
    } else {
        return `Keep practicing! Top score: ${max}`;
    }
};
...
const calculateSalePrice = (name, price) => `Produce: ${name} - Sale Price: ${price * .8}`;
